...
#Region "MAIN"
        ...
        Public Function GetList(context As IPluginContext) As PluginApi.Plugins.Playlist Implements IPlugin.GetList
            ...
            Select Case PathSpliter(PathSpliter.Length - 1)
                'Трекер Кинозал
                Case "PAGEKNZPOISK"
                    Return GetPAGEKinozal(context, "", PathSpliter(PathSpliter.Length - 2))
                Case "PAGEKNZLTOP"
                    Return GetTopRazdach(PathSpliter(PathSpliter.Length - 2), context)
                Case "PAGEKNZL"
...
#Region "Kinozal"
        ...
        'Dim TrackerServerKinozal As String = "https://kinozal-me.appspot.com"
        ...
        Function GetPAGEFilmKinozal(ByVal context As IPluginContext, ByVal ID As String)
            ...
            Dim ReGex As New Text.RegularExpressions.Regex("(?<=Подобные раздачи <a href=""/browse.php\?s=).*?(?="")")
            If ReGex.IsMatch(ResponseFromServer) = True Then
                Item = New Item
                With Item
                    .ImageLink = ICO_Search2
                    .Name = "<span style=""color:#C0E3D3"">" & "Искать ещё похожие раздачи" & "</span>"
                    .Link = ReGex.Match(ResponseFromServer).Value & ";PAGEKNZPOISK"
                End With
                items.Add(Item)
            End If
        ...
        Public Function GetTopRazdach(ByVal URL As String, context As IPluginContext) As PluginApi.Plugins.Playlist
            On Error Resume Next
            Dim items As New System.Collections.Generic.List(Of Item)
            Dim Item As New Item
            Dim RequestGet As System.Net.HttpWebRequest = Net.HttpWebRequest.CreateHttp(TrackerServerKinozal & URL)
            If ProxyEnablerKZ = True Then RequestGet.Proxy = New System.Net.WebProxy(ProxyServr, ProxyPort)
            RequestGet.Method = "GET"
            RequestGet.Headers.Add("Cookie", CookiesKNZL)
            RequestGet.ContentType = "text/html; charset=windows-1251"
...
#Region "TorrentTV"
        ...
        Public Function GetTV(ByVal context As IPluginContext) As PluginApi.Plugins.Playlist
            ...
            With Item
                ...
                .Link = "tvall"
                .ImageLink = ICO_TV
                .Description = "<html><font face="" Arial"" size="" 5""><b>" & UCase(.Name) & "</font></b><p><img width=""100%"" src=""https://tvfeed.in/img/acestream-main.jpg"">"
            End With
            items.Add(Item)
        ...
        Public Function GetTVAll(ByVal context As IPluginContext) As PluginApi.Plugins.Playlist
            ...
            Dim Category As String() = {"informational", "Информационные", "entertaining", "Развлекательные", "educational", "Познавательные", "movies", "Кино", "documentaries", "Документалистика", "fashion", "Мода", "sport", "Спортивные", "music", "Музыкальные" _
                , "regional", "Региональные", "ethnic", "Этнические", "religion", "Религиозные", "teleshop", "Телемагазин", "erotic_18_plus", "Эротические 18+", "other_18_plus", "Другие 18+", "cyber_games", "Cyber Games", "amateur", "Любительские" _
                    ...
                    With Item
                        ...
                        Else
                            ...
                            Select Case StatusIt
                                Case "2"
                                    .Description = "<html><font face=""Arial"" size=""5""><b><span style=""color:#f0ffd5"">" & .Name & "</font></b><p><img src=""" & .ImageLink & """ style=""width:40%;float:centr;""/><P>Статус: " & StatusIt
                                Case "1"
                                    .Description = "<html><font face=""Arial"" size=""5""><b><span style=""color:#fff2d5"">" & .Name & "</font></b><p><img src=""" & .ImageLink & """ style=""width:40%;float:centr;""/><P>Статус: " & StatusIt
                            End Select
